...
export class LocationMapComponent {
  ...
  onLocationChange(newEvent: AutoCompleteSelectEvent) {
    const newCountry: Country = newEvent.value as Country;
    ...
    effect(() => {
      ...
      if(countriesState.status === "OK" && countriesState.value) {
        ...
        this.toastService.send({
          severity: "error",
          summary: "Error",
          detail: "An error occurred while loading the countries"
        })
      }
    });
  }
    ...
    if(this.currentLocation) {
      this.provider!.search({ query: this.currentLocation.name.common }).then(results => {
        if(results && results.length > 0) {
          ...
          this.map!.setView(new L.LatLng(firstResult.y, firstResult.x), 10);
  ...
  search(newCompleteEvent: AutoCompleteCompleteEvent) {
      this.filteredCountries = this.countries.filter(country => country.name.common.toLowerCase().startsWith(newCompleteEvent.query.toLowerCase()));
  }
}
